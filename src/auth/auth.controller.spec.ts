...
describe('AuthController', () => {
  ...
  describe('login', () => {
    ...
    it('should throw if httpOnlyRefreshToken fails', async () => {
      ...
      mockAuthService.login.mockResolvedValue(mockResult as any);
      const mockSetCookie = jest.fn(() => { throw new Error('Cookie set failed'); });
      controller['httpOnlyRefreshToken'] = mockSetCookie;

      await expect(controller.login(mockLoginDto as any, { cookie: jest.fn() } as any)).rejects.toThrow('Cookie set failed');
    });
