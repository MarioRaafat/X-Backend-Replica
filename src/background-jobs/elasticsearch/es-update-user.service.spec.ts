...
import { EsUpdateUserJobService } from './es-update-user.service';
import { getQueueToken } from '@nestjs/bull';
import { QUEUE_NAMES } from '../constants/queue.constants';
import type { Queue } from 'bull';
import { EsSyncUserDto } from './dtos/es-sync-user.dto';

describe('EsUpdateUserJobService', () => {
    let service: EsUpdateUserJobService;
    ...
    beforeEach(async () => {
        ...
        service = module.get<EsUpdateUserJobService>(EsUpdateUserJobService);
    ...
    describe('queueUpdateUser', () => {
        it('should queue an update user job successfully', async () => {
            const dto = { user_id: 'user-123' };
            const mock_job = { id: 'job-123', data: dto };

            mock_queue.add.mockResolvedValue(mock_job as any);

            const result = await service.queueUpdateUser(dto);
        ...
        it('should queue job with custom priority and delay', async () => {
            ...
            await service.queueUpdateUser(dto, custom_priority, custom_delay);
        ...
        it('should handle queue errors', async () => {
            const dto: EsSyncUserDto = { user_id: 'user-123' };
