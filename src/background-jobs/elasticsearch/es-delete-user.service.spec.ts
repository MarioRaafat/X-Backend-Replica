...
import { EsDeleteUserJobService } from './es-delete-user.service';
import { getQueueToken } from '@nestjs/bull';
import { QUEUE_NAMES } from '../constants/queue.constants';
import type { Queue } from 'bull';
import { EsSyncUserDto } from './dtos/es-sync-user.dto';

describe('EsDeleteUserJobService', () => {
    let service: EsDeleteUserJobService;
    ...
    beforeEach(async () => {
        ...
        service = module.get<EsDeleteUserJobService>(EsDeleteUserJobService);
    ...
    describe('queueDeleteUser', () => {
        it('should queue a delete user job successfully', async () => {
            const dto = { user_id: 'user-123' };
            const mock_job = { id: 'job-123', data: dto };

            mock_queue.add.mockResolvedValue(mock_job as any);

            const result = await service.queueDeleteUser(dto);
        ...
        it('should queue job with custom priority and delay', async () => {
            ...
            await service.queueDeleteUser(dto, custom_priority, custom_delay);
